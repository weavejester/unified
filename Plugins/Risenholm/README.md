...
| `NWNX_RISENHOLM_STATIC_PLACEABLE_UPDATE_DIVISOR` | integer | 0 (default) leaves the per-player object update alone. N runs the per-object update check for static placeables on only one in N of each player's sends, staggered by object id. Unlike NWNX Optimizations' `ALTERNATE_GAME_OBJECT_UPDATE`, every object still goes through `ComputeGameObjectUpdateForObject`, so NWNX_Visibility always-visible and all NWNX_Player per-player overrides keep working; a static placeable's changes just reach the client up to N sends (N x 200 ms) late. 4 is a reasonable value. |
